            ...
            <header className="h-20 shrink-0 bg-zinc-50 dark:bg-zinc-900/40 backdrop-blur-md border-b border-zinc-200 dark:border-white/5 flex items-center px-10 relative">
                <div>
                    <h1 className="text-2xl font-black tracking-tight text-zinc-900 dark:text-white flex items-center gap-2">
                        Customer <span className="text-emerald-500">Directory</span>
                    </h1>
                    <p className="text-xs font-semibold text-zinc-500 dark:text-zinc-500 uppercase tracking-widest">Active Client Relationships</p>
                </div>
                    ...
                    <>
                        {customers.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-96 text-zinc-500 dark:text-zinc-500">
                                ...
                                <p className="text-lg font-medium text-zinc-600 dark:text-zinc-400">No Customers Found</p>
                            </div>
                        ) : (
                            <motion.div 
                                ...
                                {customers.map((customer) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center space-x-4 mb-6 relative z-10">
                                            ...
                                            <div>
                                                <h3 className="font-bold text-zinc-900 dark:text-white text-lg group-hover:text-emerald-400 transition-colors leading-tight">{customer.name}</h3>
                                                <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-600">ID: #{customer.id}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-4 relative z-10">
                                            <div className="flex items-center space-x-3 text-sm text-zinc-600 dark:text-zinc-400">
                                                ...
                                                <span>{customer.tel || 'No Contact Number'}</span>
                                            </div>
                                            {customer.email && (
                                                <div className="flex items-center space-x-3 text-sm text-zinc-600 dark:text-zinc-400">
                                            ...
                                            <div className="flex items-start space-x-3 text-sm text-zinc-500 pt-3 border-t border-zinc-200 dark:border-white/5">
                                                ...
                                                <p className="line-clamp-2 italic">{customer.address || 'Location data not available'}</p>
                                            </div>
                                        </div>
                                    </motion.div>
                                ))}
