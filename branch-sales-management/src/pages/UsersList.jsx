            ...
            <header className="h-20 shrink-0 bg-zinc-50 dark:bg-zinc-900/40 backdrop-blur-md border-b border-zinc-200 dark:border-white/5 flex items-center px-10 relative">
                <div>
                    <h1 className="text-2xl font-black tracking-tight text-zinc-900 dark:text-white flex items-center gap-2">
                        Staff <span className="text-emerald-500">Directory</span>
                    </h1>
                    <p className="text-xs font-semibold text-zinc-500 dark:text-zinc-500 uppercase tracking-widest">System Users & Administrative Access</p>
                </div>
                    ...
                    <>
                        {users.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-96 text-zinc-500 dark:text-zinc-500">
                                ...
                                <p className="text-lg font-medium text-zinc-600 dark:text-zinc-400">No Users Found</p>
                            </div>
                        ) : (
                            <motion.div 
                                ...
                                {users.map((user) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center space-x-4 mb-6 relative z-10">
                                            ...
                                            <div>
                                                <h3 className="font-bold text-zinc-900 dark:text-white text-lg group-hover:text-emerald-400 transition-colors">{user.firstName} {user.lastName}</h3>
                                                <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-600">UN: @{user.username}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-4 relative z-10">
                                            <div className="flex items-center space-x-3 text-sm text-zinc-600 dark:text-zinc-400">
                                                ...
                                                <span className="truncate">{user.email || 'No Email Record'}</span>
                                            </div>
                                            <div className="flex items-center space-x-3 text-sm text-zinc-600 dark:text-zinc-400">
                                                <PhoneIcon className="w-4 h-4 text-emerald-500/60" />
                                                <span>{user.telephone || 'No Phone Record'}</span>
                                            </div>
                                            
                                            <div className="flex items-center justify-between pt-4 border-t border-zinc-200 dark:border-white/5">
                                                <div className="flex items-center space-x-2">
                                                    <div className={`w-2 h-2 rounded-full ${user.status === 1 ? 'bg-emerald-500' : 'bg-rose-500'}`} />
